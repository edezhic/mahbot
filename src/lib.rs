...
impl DiagnosticsCommands {
    ...
    /// True if the `lint` slot holds a gate-breaking clippy compound (which
    /// [`normalize_gate_breaking_clippy_compound`] will rewrite into a fix+gate
    /// chain) **and** the compound's fix pass is exactly the stored `lint-fix`
    /// command — i.e. the separate `lint-fix` slot is redundant and would run
    /// the same fix twice.
    ///
    /// The equivalence check matters: when the stored `lint-fix` carries
    /// configuration the compound's fix lacks (e.g. `--all-targets`), skipping
    /// it would silently drop that coverage, so `false` is returned and both
    /// slots run.
    #[must_use]
    pub fn lint_slot_merges_lint_fix(&self) -> bool {
        let (Some(lint), Some(lint_fix)) = (self.lint.as_deref(), self.lint_fix.as_deref()) else {
            return false;
        };
        let Some(chain) = normalize_gate_breaking_clippy_compound(lint) else {
            return false;
        };
        // The only `&&` in a normalized chain is the separator we add
        // (detection rejects already-compound commands), so this split is
        // unambiguous.
        chain
            .split_once(" && ")
            .is_some_and(|(fix_pass, _)| fix_pass == lint_fix)
    }
}
...
/// pass with `-D` lint gates in the trailing `--` rustc args.
///
/// On the pinned toolchain (rustc/cargo 1.97.1) the fix driver silently drops
/// the lint-level configuration: the compound exits 0 with unfixable warnings
/// still present (the lint itself reports "ignores `-D warnings`"), so a
/// stored/generated command of this shape can silently disable the
/// `-D warnings` gate that the diagnostics phase is supposed to enforce.
///
/// Detection is deliberately restricted to clippy's known broken pattern:
/// a single invocation mentioning `clippy` with both `--fix` and a
/// `-D`/`--deny` gate. Commands that are already compound (`&&`, `;`,
/// newline) are never rewritten, and arbitrary custom commands are left
/// untouched.
#[must_use]
pub(crate) fn is_gate_breaking_clippy_compound(cmd: &str) -> bool {
    if cmd.contains("&&") || cmd.contains(';') || cmd.contains('\n') {
        return false;
    }
    let tokens: Vec<&str> = cmd.split_whitespace().collect();
    let mentions_clippy = tokens.iter().any(|t| t.contains("clippy"));
    let has_fix = tokens.iter().any(|t| t.starts_with("--fix"));
    let has_gate = trailing_rustc_args(&tokens)
        .is_some_and(|after| after.iter().any(|t| is_lint_gate_flag(t)));
    mentions_clippy && has_fix && has_gate
}

/// Rewrite a gate-breaking clippy compound into the gate-preserving two-pass
/// chain: fix pass (no gate) `&&` gate pass (no fix).
///
/// ```text
/// cargo clippy --fix --allow-dirty -- -D warnings
///   → cargo clippy --fix --allow-dirty && cargo clippy -- -D warnings
/// ```
///
/// The gate pass drops the fix-only flags ([`FIX_ONLY_CLIPPY_FLAGS`]) that a
/// plain `cargo clippy` invocation rejects with a usage error (exit 1,
/// verified on the pinned 1.97.1 toolchain) — keeping them would make the
/// repaired chain fail on every run, even on lint-free code, and the `-D
/// warnings` gate would never actually execute. Value-taking fix-only flags
/// (e.g. `--edition …`) are out of scope for the repair: the gate pass would
/// fail loudly rather than silently disable the gate, which is the safe
/// direction.
///
/// Returns `None` when `cmd` is not a gate-breaking compound (the command is
/// executed exactly as stored).
#[must_use]
pub(crate) fn normalize_gate_breaking_clippy_compound(cmd: &str) -> Option<String> {
    if !is_gate_breaking_clippy_compound(cmd) {
        return None;
    }
    let tokens: Vec<&str> = cmd.split_whitespace().collect();
    let sep = tokens.iter().position(|t| *t == "--")?;
    let (before, after) = tokens.split_at(sep);
    // Detection guarantees a non-empty trailing gate segment, so `after[1..]`
    // is always at least the gate flag(s).
    let after = &after[1..]; // drop the `--` separator itself
    let fix_pass = before.join(" ");
    let gate_pass: Vec<&str> = before
        .iter()
        .filter(|t| !t.starts_with("--fix") && !FIX_ONLY_CLIPPY_FLAGS.contains(t))
        .copied()
        .collect();
    Some(format!(
        "{fix_pass} && {} -- {}",
...
/// Cargo flags that are only valid on the fix pass (`cargo clippy --fix` /
/// `cargo fix`) and are rejected by a plain `cargo clippy` invocation
/// ("unexpected argument", exit 1 — verified on rustc/cargo 1.97.1).
///
/// When [`normalize_gate_breaking_clippy_compound`] rewrites a gate-breaking
/// compound into the two-pass chain, these must be stripped from the gate
/// pass or the repaired chain fails on every run, even on lint-free code.
const FIX_ONLY_CLIPPY_FLAGS: &[&str] = &[
    "--allow-dirty",
    "--allow-no-vcs",
    "--allow-staged",
    "--broken-code",
...
/// The tokens after a standalone `--` separator, if any.
fn trailing_rustc_args<'a>(tokens: &'a [&'a str]) -> Option<&'a [&'a str]> {
    let sep = tokens.iter().position(|t| *t == "--")?;
    Some(&tokens[sep + 1..])
}

/// True when a token is a `-D`/`--deny` lint-gate flag (rustc deny form).
fn is_lint_gate_flag(tok: &str) -> bool {
...
mod tests {
    ...
    #[test]
    fn gate_breaking_compound_detection() {
        // The canonical broken compound (rust-clippy#17444): fix + gate in one
        // invocation. Verified on rustc/cargo 1.97.1 to exit 0 with the gate
        // silently disabled.
        assert!(is_gate_breaking_clippy_compound(
            "cargo clippy --fix --allow-dirty -- -D warnings"
        ));
        assert!(is_gate_breaking_clippy_compound(
        ...
        assert!(is_gate_breaking_clippy_compound(
            "cargo clippy --fix --allow-dirty -- -D warnings,clippy::all"
        ));
        assert!(is_gate_breaking_clippy_compound(
            "cargo clippy --fix -- -D warnings --no-deps"
        ));
        assert!(is_gate_breaking_clippy_compound(
            "cargo clippy --fix -- --deny warnings"
        ));

        // Not broken: fix without a gate (the standard two-slot shape).
        assert!(!is_gate_breaking_clippy_compound(
            "cargo clippy --fix --allow-dirty"
        ));
        // Not broken: gate without a fix pass.
        assert!(!is_gate_breaking_clippy_compound(
            "cargo clippy -- -D warnings"
        ));
        // Not broken: non-clippy tool with the same flag shape.
        assert!(!is_gate_breaking_clippy_compound(
            "cargo fix --allow-dirty -- -D warnings"
        ));
        // Not broken: already a compound chain — never rewritten.
        assert!(!is_gate_breaking_clippy_compound(
            "cargo clippy --fix --allow-dirty && cargo clippy -- -D warnings"
        ));
        // Not broken: no `--` rustc-arg separator.
        assert!(!is_gate_breaking_clippy_compound(
            "cargo clippy --fix --allow-dirty -D warnings"
        ));
        // Not broken: `--fix` written as a glued cargo flag is still a fix,
        // but without a standalone `--` separator there is no rustc gate.
        assert!(!is_gate_breaking_clippy_compound(
            "cargo clippy --fix --allow-dirty --bin=app"
        ));
    }

    #[test]
    fn gate_breaking_compound_normalization() {
        // Fix pass keeps --fix (and drops the gate); gate pass drops --fix and
        // the fix-only flags (--allow-dirty is invalid for a plain clippy
        // invocation — "unexpected argument", exit 1 on 1.97.1). The chain
        // preserves both passes and the gate.
        assert_eq!(
            normalize_gate_breaking_clippy_compound(
                "cargo clippy --fix --allow-dirty -- -D warnings"
            ),
            Some("cargo clippy --fix --allow-dirty && cargo clippy -- -D warnings".to_string())
        );
        assert_eq!(
            normalize_gate_breaking_clippy_compound("cargo clippy --fix -- -D warnings"),
            Some("cargo clippy --fix && cargo clippy -- -D warnings".to_string())
        );
        // All fix-only flags are stripped from the gate pass but kept in the
        // fix pass; non-fix-only clippy flags are kept in both passes.
        assert_eq!(
            normalize_gate_breaking_clippy_compound(
                "cargo clippy --fix --allow-dirty --allow-no-vcs --allow-staged --broken-code -- -D warnings"
            ),
            Some(
                "cargo clippy --fix --allow-dirty --allow-no-vcs --allow-staged --broken-code \
                 && cargo clippy -- -D warnings"
                    .to_string()
            )
        );
        // Gate args are preserved verbatim in the gate pass.
        assert_eq!(
            normalize_gate_breaking_clippy_compound(
                "cargo clippy --fix --all-targets -- -D warnings --no-deps"
            ),
            Some(
                "cargo clippy --fix --all-targets && cargo clippy --all-targets -- -D warnings --no-deps"
                    .to_string()
            )
        );

        // Non-compound commands pass through untouched.
        assert_eq!(
            normalize_gate_breaking_clippy_compound("cargo clippy -- -D warnings"),
            None
        );
        assert_eq!(
            normalize_gate_breaking_clippy_compound("cargo clippy --fix --allow-dirty"),
            None
        );
        assert_eq!(
            normalize_gate_breaking_clippy_compound(
    ...
    #[test]
    fn lint_slot_merge_detection() {
        // Merge: the lint slot is a gate-breaking compound whose fix pass is
        // exactly the stored lint-fix — the lint-fix slot is redundant.
        let merged = DiagnosticsCommands {
            lint_fix: Some("cargo clippy --fix --allow-dirty".into()),
            lint: Some("cargo clippy --fix --allow-dirty -- -D warnings".into()),
            ..Default::default()
        };
        assert!(merged.lint_slot_merges_lint_fix());

        // No merge when the stored lint-fix carries extra config the
        // compound's fix lacks — skipping it would drop coverage.
        let extra_config = DiagnosticsCommands {
            lint_fix: Some("cargo clippy --fix --all-targets".into()),
            lint: Some("cargo clippy --fix --allow-dirty -- -D warnings".into()),
            ..Default::default()
        };
        assert!(!extra_config.lint_slot_merges_lint_fix());

        // No merge when lint is not a gate-breaking compound (plain two-pass).
        let plain = DiagnosticsCommands {
            lint: Some("cargo clippy -- -D warnings".into()),
            lint_fix: Some("cargo clippy --fix --allow-dirty".into()),
            ..Default::default()
        };
        assert!(!plain.lint_slot_merges_lint_fix());

        // No merge when either slot is unset.
        let unset = DiagnosticsCommands::default();
        assert!(!unset.lint_slot_merges_lint_fix());
    }
    #[test]
