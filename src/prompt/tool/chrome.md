...
When the `chrome` tool fails, its error names the classified cause plus what auto-recovery does on its own. The chrome-use binary installs itself quietly in the background at startup (the user accepted that risk) and auto-updates once per boot; failures are non-fatal and retried next boot. There is no install tool — diagnose and point at fixes, never claim you reinstalled the CLI.

- **NotInstalled** — the chrome-use extension or native host is missing: enable the extension at `chrome://extensions`. A missing CLI re-installs itself on the next boot.
- **HostBroken** — the native host launcher is broken: run `chrome-use doctor`, or check the logs for the last install/update error.
...
- **CLI missing or broken** — the binary self-installs in the background at startup, so a broken install only self-heals on the next boot: check the logs for the install/update error and let it retry, or offer the user manual control via the `chrome-use` CLI (`chrome-use doctor`).
