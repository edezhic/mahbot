...
- Sessions are strictly scoped to the agent that started them: no other agent can read or stop them, and when your run ends — success, error, or abort — the session's process tree is destroyed. Never assume a background process survives past your run — if you need it later, restart it.
- Launch failures (command not found, not executable) are returned as a tool error immediately — check the tool response before assuming the session started. That check is Unix-only: on Windows a missing or non-executable command starts a session whose file holds the interpreter's error and ends with `[exit status: 1]`.
