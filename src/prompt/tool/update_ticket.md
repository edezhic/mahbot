Transition an existing ticket to a new phase. Requires the ticket `ticket_id`, which you can obtain from the output of create_ticket or list_tickets. The valid phases for manual transitions are:
...
In general you should not set any other phases unless explicitly requested by the user.