    ...
    fn parameters_schema(&self) -> serde_json::Value {
        super::tool_params_schema(
            &serde_json::json!({
                "query": {
                    "type": "string",
                    "description": "A single read-only SQL statement (multi-statement input is rejected). Any mutating statement or non-read-only PRAGMA is rejected before execution — see the tool description for the full blocklist."
                },
                "db": {
                    "type": "string",
                    "description": format!(
